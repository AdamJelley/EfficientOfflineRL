...
class Actor(nn.Module):
    ...
    def forward(
        ...
        if deterministic:
            action = mu
            # with torch.no_grad():
            #     actions = torch.stack([policy_dist.rsample() for i in range(100)], dim=0)
            #     states = state.repeat(100, 1)
            #     currentQ = critic(states, actions).mean(dim=0, keepdim=True)
            #     index = currentQ.max(dim=1).indices
            #     action = actions[index].squeeze()
        else:
...
class SVGD:
    def __init__(self, kernel, optimizer, lmbda):
        self.K = kernel
    ...
    def step(self):
        ...
        for param, phi in zip(
            ...
            torch.split(self.phi, self.param_lengths, dim=1),
        ):
...
def train(config: TrainConfig):
    ...
    critic_optimizer = torch.optim.Adam(
        critic.parameters(), lr=config.critic_learning_rate  # , weight_decay=0.01
    )
    SVGD_critic_optimizer = SVGD(
        kernel=RBF(), optimizer=critic_optimizer, lmbda=config.lmbda
    )
    ...
    trainer = SASVC(
        ...
        critic=critic,
        critic_optimizer=SVGD_critic_optimizer,
        gamma=config.gamma,
        tau=config.tau,
        eta=config.eta,
